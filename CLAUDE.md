...
- The Likert scale column geometry lives in four custom properties on `.likert-blok` — `--skala-kolona` (column pitch), `--skala-dugme` (button width), `--skala-razmak` (gap) and `--skala-oznaka` (label font size) — read by both `.likert-brojevi span` and `.likert-opcija`, which is what keeps each header number directly above its button. Two rules follow from that: **`.likert-brojevi span small` must never get a `max-width` larger than `--skala-kolona`** (a fixed 62px label in a 52px column is what made neighbouring labels overlap), and **`--skala-kolona` must be wider than the longest single word in any label** — the widest is "preferiram," at 61.4px, hence 66px. Since column and font size have to change together, both are fixed values on purpose: `vw`-based `clamp()` scales them at different rates and the column collapses under the word on mid-width screens. `.likert-blok.likert-7` overrides all four for the 7-point SWLS scale. Because the labelled scale is wider than the text column, `.likert-blok` bleeds out of the form padding with a negative `margin-inline` (−20px, −14px at ≤1024px, 0 on the mobile pivot where the scale goes fluid).
