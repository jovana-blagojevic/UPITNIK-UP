...
⚠️ **Bump the `?v=` on the asset links after every change to `style.css` or `script.js`.** All three pages load them as `assets/style.css?v=…`. GitHub Pages serves assets with `Cache-Control: max-age=600` and browsers hold them far longer than that in practice, so without a new query string a respondent who already opened the survey keeps the old CSS — a fixed layout bug appears unfixed to exactly the people already filling the form. One shared version token across `style.css`, `script.js` and `config.js` in `index.html` and both `strane/index-*.html`.
...
**Three constraints that look like stylistic choices but are not:**
...
- The Likert scale column geometry lives in custom properties on `.likert-blok` — `--skala-oznaka` (label font size), `--skala-faktor`, `--skala-kolona` (column pitch, **derived**: `calc(var(--skala-oznaka) * var(--skala-faktor))`), `--skala-dugme` (button width) and `--skala-razmak` (gap) — read by both `.likert-brojevi span` and `.likert-opcija`, which is what keeps each header number directly above its button. Three rules follow from that:
  - **`--skala-kolona` must stay wider than the longest single word in any label.** The widest is "preferiram," at 5.33× the label font size, hence the 5.9 factor (~3px of air each side). Deriving the column from the font size instead of hard-coding both in px is what survives a reader who raised their browser's default font size: the label is in `rem` and grows, so the column has to grow with it — that mismatch is what made neighbouring labels touch. `vw`-based `clamp()` is the opposite failure: it scales column and font at different rates and the column collapses under the word on mid-width screens.
  - **`.likert-brojevi span small` must never get a `max-width` larger than `--skala-kolona`** (a fixed 62px label in a 52px column is the original form of the same overlap bug).
  - **`.likert-zaglavlje` and `.likert-red` must both use `grid-template-columns: minmax(0, 1fr) auto`, never bare `1fr`.** When the scale stops fitting, a bare `1fr` text column stops at its own min-content — different in the header than in a row — so the two rows overflow by different amounts and the numbers slide off their buttons.

  `.likert-blok.likert-7` overrides only `--skala-oznaka`, `--skala-dugme` and `--skala-razmak` for the 7-point SWLS scale; the column follows from the shared `calc`. Because the labelled scale is wider than the text column, `.likert-blok` bleeds out of the form padding with a negative `margin-inline` (−20px, −14px at ≤1024px, 0 on the mobile pivot where the scale goes fluid).
